...
public class Gaus implements AuctionBehavior {
...
//		cost /= myTaskList.size();
		...
		listOfMyCost.add((long) sumCost);
		
		
		
		for (int i = 0; i<listOfMyCost.size();i++) {
			sumCost+= listOfMyCost.get(i);
		}
		sumCost/=listOfMyCost.size();

		
		
		
		double bid =  sumCost + rewardGen(cost, task);
		System.out.println("new cost " +bid +" vs " + cost);
		System.out.println("Cost per Task " + (bestCoord.getTotalCost()- previousCost) );

		
		listOfMyBid.add((long) bid);
	...
	private Double rewardGen(double cost, Task task) {
		...
		if (!listOfEnemyBid.isEmpty()) {
			...
			double meanBid = 0,varianceBid = 0;
			double meanCost = 0, varianceCost =0;
			...
			System.out.println("MEEEEEEEEEEEE"+dynamicVar);

			ArrayList<Double> myWeight = new ArrayList<Double>(
			WeightedMean(listOfEnemyBid.size(), sign*Math.sqrt(dynamicVar), listOfEnemyBid.size()));
			
			
			for (int i = 0; i < listOfEnemyBid.size(); i++) {
				meanBid += (listOfEnemyBid.get(i)-listOfMyBid.get(i))*myWeight.get(i);
				meanCost += (listOfMyBid.get(i))*myWeight.get(i);
				offset1 += listOfMyBid.get(i)*myWeight.get(i);
				offset2 += listOfMyCost.get(i)*myWeight.get(i);
			}

//			meanBid/=listOfEnemyBid.size();
//			meanCost/=listOfEnemyBid.size();
//			offset1/=listOfEnemyBid.size();
//			offset2/=listOfEnemyBid.size();

			
			
			for (int i = 0; i < listOfEnemyBid.size(); i++) {
				varianceBid += Math.pow((listOfEnemyBid.get(i)-listOfMyBid.get(i))*myWeight.get(i)-meanBid,2);
				varianceCost += Math.pow((listOfMyBid.get(i))*myWeight.get(i)-meanCost,2);
			}
			varianceBid/= listOfEnemyBid.size();
			varianceCost/= listOfEnemyBid.size();
			...
			return InterGaus(meanBid,meanCost,varianceBid,varianceCost);

		} else
			return 0.0;
	}
	
	private Double InterGaus(double mean1, double mean2, double variance1, double variance2) {
		
		double a = 1/(2*variance1)-1/(2*variance2);
		double b = mean2/variance2 -mean1/variance1;
		double c = Math.pow(mean1, 2)/(2*variance1) -  Math.pow(mean2, 2)/(2*variance2) - Math.log(Math.sqrt(variance2/variance1));
...
//		
//		ArrayList<Double> myWeight = new ArrayList<Double>(
//				WeightedMean(listOfEnemyBid.size(), sign*Math.sqrt(dynamicVar), listOfEnemyBid.size()));//sign*Math.sqrt(Math.abs(dynamicVar)/2)));//5.0));//Math.sqrt(listOfEnemyBid.size()/dynamicVariance()/2)));
//
//		for (int i = 0; i < listOfEnemyBid.size(); i++) {
//			meanbid += (-listOfMyBid.get(i) + listOfEnemyBid.get(i)) * myWeight.get(listOfEnemyBid.size() - 1 - i);
//		}
//
//		for (int i = 0; i < listOfEnemyBid.size(); i++) {
//			variancebid += Math.pow((meanbid - (-listOfMyBid.get(i) + listOfEnemyBid.get(i))*myWeight.get(listOfEnemyBid.size()-1-i)), 2);
//		}
//		variancebid /=listOfEnemyBid.size();
//		
//		System.out.println("	meanbid : " + meanbid);
//
//		System.out.println("	Variance : " + variance);
//
//		return (meanbid);
