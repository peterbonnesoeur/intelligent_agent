...
public class AuctionTemplateS implements AuctionBehavior {
...
//	ArrayList<Task> myTasks = new ArrayList<Task>();
	...
	public List<Plan> plan(List<Vehicle> vehicles, TaskSet tasks) {
		...
		profit = reward-bestCost;
		System.out.println("nb tasks: " + nbTasks);
	...
	private double bid(Task task, double marginalCost, double opponentMarginalCost) {
		...
		double meanDistance = totalDistance/nbCities;
		
		double predictionError = 0;
		double relativePredictionError = 0;
		
		double bidRatio = 0;
		
		double weightFactor = meanDistance;
		double discountFactor = 0.1;
		double plusFactor = 0.3;
		
		//CALCULATE THE PREDICTION ERRORS ---------------------------------------------------------//
		for(i = 0; i < bidList.size();i++) {
			
			if(agent.id() == 1) {
				predictionError += (bidList.get(i)[0] - predictedBids.get(i))/bidList.size();
				relativePredictionError += Math.abs((bidList.get(i)[0] - predictedBids.get(i)))/((double)bidList.size()*bidList.get(i)[0]);
			}
			else {
				predictionError += (bidList.get(i)[1] - predictedBids.get(i))/bidList.size();
				relativePredictionError += Math.abs((bidList.get(i)[1] - predictedBids.get(i)))/((double)bidList.size()*bidList.get(i)[1]);
			}			
		}
		//-----------------------------------------------------------------------------------------//
		...
		//ADD OR SUBSTRACT BASED ON THE CITIES OF THE TASK -----------------//
		if(nbTasks < 5) {
			if(cityWeights[task.pickupCity.id] < meanCityWeight) {
				bid += weightFactor*cityWeights[task.pickupCity.id];
				opponentBid += weightFactor*cityWeights[task.pickupCity.id];
			}
			else {
				bid -= weightFactor*cityWeights[task.pickupCity.id];
				opponentBid -= weightFactor*cityWeights[task.pickupCity.id];
			...
			if(cityWeights[task.deliveryCity.id] < meanCityWeight) {
				bid += weightFactor*cityWeights[task.deliveryCity.id];
				opponentBid += weightFactor*cityWeights[task.deliveryCity.id];
			}
			else {
				bid -= weightFactor*cityWeights[task.deliveryCity.id];
				opponentBid -= weightFactor*cityWeights[task.deliveryCity.id];
		...
		// NO NEG BIDS --------------------------------------------------------//
		if(bid < 100) {
			bid = task.pickupCity.distanceTo(task.deliveryCity)*myVehicles.get(0).costPerKm();
			if(bid <500)
				bid = 750;
		}
		//---------------------------------------------------------------------//
		...
		//CALCULATE RATIO OF BIDS ---------------------------------------------//
		for(i = task.id-1; i > task.id-5 && i >= 0; i--) {
			...
			bidRatio += (theirBid-myBid)/(myBid);
		}
		
		if(task.id <5 && task.id != 0)
			bidRatio = bidRatio/bidList.size();
		else
			bidRatio = bidRatio/5;
		//-----------------------------------------------------------------------//
		...
		//AJUST BIDS TO RATIO ---------------------------------------------------//
		if(bidRatio > 0) {
			bid = (1+plusFactor)*bid;
			opponentBid = (1-discountFactor)*opponentBid;
		}
		else {
			bid = (1-discountFactor)*bid;
			opponentBid = (1+plusFactor)*opponentBid;
		}
		//-----------------------------------------------------------------------//
		
		
		
		//HIGHER BIDS IF ABLE TO PREDICT CORRECTLY ------------------------------//
		if(relativePredictionError < 0.1 && 0.75*(opponentBid+predictionError) > bid && task.id >= 5) {
			bid = 0.75*(opponentBid+predictionError);
		}
		//-----------------------------------------------------------------------//
		
		
		//GET AT LEAST TWO TASKS IN THE BEGINNING -------------------------------//
		if(task.id < 10 && nbTasks < 2) {
			bid = bid*((10.0 - (double) task.id)/10.0);
		}
		//-----------------------------------------------------------------------//
	...
	private void  tablesInit() {
		...
		for(i = 0;i<nbCities;i++) {
			...
			for(j = 0;j < nbCities;j++) {
				if(i!=j) {
					cityWeights[i] += distribution.probability(myCities.get(i), myCities.get(j))/(myCities.get(i).distanceTo(myCities.get(j)))/nbCities;
				}
			}
