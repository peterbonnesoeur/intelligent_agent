...
public class AuctionTemplate implements AuctionBehavior {
	...
	public Long askPrice(Task task) {
		...
		if (bid < 1.05 * Math.abs(cost)) {
			return (long) Math.round(1.05 * Math.abs(cost));
		}
	...
	private double costCalculator(Coordination bestCoord,int size) {
		...
		double totalCost = bestCoord.getTotalCost()/size;		// our strategy is here to compute the mean cost which is the cost divided by the number of tasks
