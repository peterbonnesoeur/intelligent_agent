...
public class CentralizedTemplate implements CentralizedBehavior {
    ...
    public List<Plan> plan(List<Vehicle> vehicles, TaskSet tasks) {
		...
		switch (algorithm) {
			case NAIVE : 
				...
				break;
			case TYPE1: 
				plans.addAll(optimizedPlanV1(vehicles,tasks));
				break;
			case TYPE2:
				break;
			default:
    ...
    private List<Plan> optimizedPlanV1(List<Vehicle> vehicles, TaskSet tasks) {
    		...
    		System.out.println("we are before creating a coordination " );
    ...
    private class Coordination{
    		...
    		public Coordination(List<Vehicle> myVehicles, TaskSet myTasks) {
    			...
    			for(Vehicle V:myVehicles)
    			{
    				...
    				System.out.println("we added the sequence to the hashMap" );
    				iterV++;
    			}
    		...
    		public List<Plan> getPlans(){
    			...
    			for(Vehicle V : vehicles) {
    	    		...
    	    		System.out.println("Our vehicle (asumption) : " + V );
    	    		System.out.println("Our vehicle (real) : " + sequences.get(V).seqVehicle );
    	    		System.out.println("City pf the vehicle : " + sequences.get(V).seqVehicle.getCurrentCity() );
    			}
    			return plans;
    		}
    ...
    private class Sequence{
	    	...
	    	public Vehicle seqVehicle=null;														// TO MODIFY
	    	...
	    	public Sequence(ArrayList<Task> seqTasks,Vehicle currentVehicle) {
	    		...
	    		if(seqTasks!=null)
	    		{
	    			...
	    			{
	    				System.out.println("we are creating the group "+i );
	    			...
	    			for(Task task:seqTasks) 
	    			{
	    				...
	    				int i = new Random().nextInt((this.groups.size()));
	    				System.out.println("to group "+i);
	    				this.groups.get(i).addTask(task);
	    				
	    				System.out.println(": "+this.groups.get(i));
	    			}
	    			System.out.println("the groups are "+this.groups );
	    		}
	    	}
	    		...
	    		for(int i=0;i<seqEvents.size();i++)
	    		{
	    			...
	    			if(myEvent.getStatus()==Status.PICKUP)
	    			{
	    				...
	    				System.out.println("pickUp City: "+ myEvent.getTask().pickupCity);
	    					...
	    					{	
	    						System.out.println("Target city  : "+ path);
	    						seqPlan.appendMove(path);
	    					}
	    				seqPlan.appendPickup(myEvent.getTask());
	    				
	    				myCity = myEvent.getTask().pickupCity;
	    				System.out.println("NewCity : "+ myCity);
	    				
	    			}else if(myEvent.getStatus()==Status.DELIVER)
	    			{
    ...
    private class Group{
    		...
    		public boolean addTask(Task task) {
    			...
    			for(int i=0; i< groupEvents.size();i++)
    			{
    				System.out.println("task: "+groupEvents.get(i).getTask()+" status: "+groupEvents.get(i).getStatus());
    			}
    			...
    			tempEvents.add(new Event(task,Status.PICKUP,tempEvents.get(pickupOrder).getWeight()+task.weight));
    			...
    			if(range!=0)
    			{
    				deliveryOrder = deliveryOrder+new Random().nextInt(range);
    			}
    			
    			tempEvents.add(new Event(task,Status.DELIVER,tempEvents.get(deliveryOrder).getWeight()));
